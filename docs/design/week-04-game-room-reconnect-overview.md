...
- 최초 작성일: 2026-08-21
- 최근 검토일: 2026-09-10
- 구현 상태: 핵심 코드 구현 — 전투·연결·재접속·만료 복구·인증된 HTTP 경로 연결, 개발 DB 적용·커밋·CI 확인은 별도
- 상위 계획: 4주차 — 게임 룸과 재접속

## 1. 문서 목적

3주차에서 구현한 `GameRoomGrain`은 정확히 4명을 배정하고 `Ready → InGame → Completed` 상태를 저장한다.
이후 4주차에서 게임 결과·정책 버전 저장, PlayerGrain 보상 전달, 실패한 결과의 자동 재시도를 구현했다.
웨이브 설정과 첫 웨이브 초기화·진행 값 저장을 추가했다. 공격·자동 웨이브 진행·쿨다운 검사·연결 이탈·재접속은 아직 구현하지 않았다.
...
세부 문서는 구현 직전 해당 범위를 검토하고 사용자 승인에 따라 진행한다. 문서 상태가 `제안됨`인 것과 이미 승인받아 구현한 기능을 혼동하지 않는다. 본문에는 향후 설계가 포함되므로 구현 여부는 상단 상태와 아래 진행 기준선을 함께 확인한다.
...
- 이미 저장된 `Completed`의 승패·정책 버전·완료 시각·보상 전달 이력은 보존한다. 전투 필드가 없다는 이유로 `Cancelled`로 일괄 변경하지 않는다. 전투 상세가 없는 과거 완료 기록의 호환 규칙과 진행 중 방의 사전 검사는 [세부 설계 02의 13.5절](week-04-02-game-room-combat-state-machine.md#135-기존-데이터-migration과-backfill)을 따른다.
...
- 요청 이력은 이번 명령이 만든 한 건만 추가한다. 재전송과 키 충돌은 최초 행을 수정하지 않는다. 전체 삭제·재삽입 제거는 전투 전 선행 작업으로 반영했다. 다만 후보 상태의 메모리 복제와 활성화 시 전체 이력 읽기는 여전히 남아 있으며, 이번 변경이 모든 누적 비용을 제거한 것은 아니다.
...
### 최신 상태 — 2026-09-11 연결·HTTP 구현 후

전투 연결 자격 검사, Connect·Heartbeat·Reconnect·Disconnect·StartCombat, 연결 상태 영속화, 최초 입장 취소, 유예 만료·전원 포기 패배, 타이머·복구 작업자, 실제 JWT HTTP 테스트를 추가했다. 함수별 변경 이유·실행 경로·검증 범위·설계와 실제 구현의 차이는 [4주차 구현 검토](week-04-implementation-review.md)에 정리했다. 운영 설정에서 정책값을 바꾸는 연결과 다중 API 분산 호출 제한은 운영 확장으로 남긴다.

아래 날짜별 구현 기록은 당시의 상태이며, 미구현이라고 적힌 내용은 위 최신 상태로 대체한다.

### 2026-09-11 후속 구현 상태

최종 검증: Release 단위 테스트 102개·통합 테스트 117개, 합계 219개 통과. 모델과 마이그레이션 차이 없음. Docker 종료로 발생한 환경 실패는 엔진 재시작 후 전체 재실행하여 해소했다. 기존 기록 비교 테스트는 새 열을 별도 검증하며 원래 데이터 비교를 유지했다.

- 서버 내부 Orleans 전투 계약을 연결했다. 공격 후보·참가자·요청·승인 순번·최종 결과 전달 행은 동일 DB 트랜잭션에서 저장하며 커밋 뒤에만 메모리를 교체한다.
- 실제 매칭을 거쳐 3인 파티+솔로가 3웨이브를 완료하고, 보상 전달·파티 유지·후처리 재생을 검증했다. HTTP 공격 경로는 연결 권한 검사 전이므로 아직 공개하지 않았다.
- `FinalizationPending`으로 전투 완료 후 파티·티켓 복귀의 미완료 상태를 보존한다. 방별 결정적 하위 요청을 사용해 새 매칭 이후에도 과거 후처리는 최초 결과만 재생한다. 복구 처리기는 이 표식이 남은 방도 조회한다.
- 연결 유지 15초·재접속 유예 30초, 이전 연결 차단, 연결 교체 경쟁, 기한 경계의 순수 규칙을 추가했다. 아직 이 연결 규칙을 참가자 DB 열·Grain·HTTP에 연결하지 않았다.
- 따라서 4주차 전체 완료가 아니다. 연결 영속화·Heartbeat·재접속·만료 평가·참가자 Start·인증된 공격 API·전체 재접속 흐름 검증이 남아 있다. 기존 관리자 진단 경로는 계속 유지한다.
- 생성한 마이그레이션은 테스트 DB에만 적용했으며 개발 DB 적용·커밋·푸시는 별도다.

2026-09-10 기준으로 실제 구현 순서를 다음과 같이 정리한다.

1. 완료: PlayerGrain·PostgreSqlRewardWriter 책임 분리 및 보상 정책
2. 완료: GameOutcome·정책 버전·플레이어별 결과 전달 상태 저장, 자동 재시도 서비스
3. 완료: 기존 결과 보존 설계 보완, 요청 이력 한 건 추가 방식과 회귀 테스트
4. 완료: 참가자 네 행의 상태 저장·복원과 기존 데이터 호환 Migration. 테스트 검증 및 2026-09-10 개발 DB 적용 완료
5. 이번 단계: 웨이브·적 체력·상태 버전 모델. 다음은 공격·스킬과 서버 시간·쿨다운·명령 순번 검증 및 자동 전투 진행
6. 이후: 연결 이탈·유예 시간·재접속
7. 이후: 전체 흐름·재시작·실패 검증 및 README·Notion·설계문서 갱신

현재 자동 복구 서비스는 `game_results`의 보상 전달을 재개한다. Party·Ticket 후처리의 영속 상태와 통합 복구는 아직 향후 설계이며, 모두 구현된 것으로 해석하지 않는다.
