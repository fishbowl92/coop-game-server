...
- 문서 상태: 제안됨(Proposed)
- 최초 작성일: 2026-08-21
- 구현 상태: 순수 규칙·DB·Grain·HTTP 연결, 만료 타이머·복구 및 재시작 검증 구현
...
최신 구현은 [4주차 구현 검토](week-04-implementation-review.md)를 참고한다. 아래 초기 규칙 구현 기록은 후속 통합 전 상태다. 실제 연결 요청은 개별 Grain 메서드 대신 `ExecuteConnectionAsync`와 Action으로 구분하며, DB 요청 종류는 `Connection`으로 묶고 JSON에 세부 명령을 보존한다. 정책 클래스의 생성자와 기본값은 구현했지만 운영 설정 파일 연결은 아직 하지 않았다. 이 차이를 완료된 운영 기능으로 해석하지 않는다.

### 2026-09-11 규칙 구현 기록

`GameRoomConnectionRules`는 생성자로 Lease·Grace 시간을 받는다. `Connect`·`Reconnect`는 서버가 생성한 새 ID를 매개변수로 받고, 순수 계산 안에서 네트워크·DB·현재 시각을 읽지 않는다. `Heartbeat`는 유효한 현재 자격만 연장한다. `Disconnect`는 명시적 종료 시각부터 유예를 시작하고, `Evaluate`는 자동 만료 시 원래 Lease 시각부터 유예를 계산한다. 따라서 평가가 늦어져도 복귀 가능 시간이 늘어나지 않는다.

`RoomPlayerConnection`은 연결 상태·ID·세대·절대 시각을 가진 불변 레코드다. `RoomConnectionTransition`은 오류와 후보 상태를 함께 반환한다. 거부 응답이어도 시간 경과로 만료된 후보가 있을 수 있으므로 향후 방 계층은 후보를 DB에 저장한 뒤 채택해야 한다. 이 후보를 현재 상태에 직접 덮어쓰는 코드는 아직 없다.

`CanReplayCredential`은 최초 연결 결과를 다시 보여줘도 되는지 판단한다. 이미 교체됐거나 만료된 자격은 반환하지 않는다. `Close`는 완료 방의 자격을 제거하되 Abandoned 기록은 보존한다. 체력·쿨다운·보상은 연결 규칙의 책임이 아니며 여기서 초기화하지 않는다.

새 단위 테스트 10개는 15초·45초 경계, 늦은 평가, Heartbeat, 경쟁 재접속, 낡은 연결 차단, 자격 재생, 시간·세대 상한을 검증한다. 아직 DB 저장·서버 재접속 또는 HTTP 보안을 검증한 것은 아니다. 아래 계약·열·타이머·작업자 연결은 남은 구현 설계다.
